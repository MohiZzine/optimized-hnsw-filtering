...
public:
    NaiveFilter();
    NaiveFilter(const std::vector<unsigned int>& query_attributes);
...
private:
    // Data storage: point_id -> set of attributes
    std::unordered_map<hnswlib::labeltype, std::unordered_set<unsigned int>> point_attributes_;
